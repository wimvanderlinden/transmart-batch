    ...
    private String createPartition(int partitionId) {
        ...
        if (primaryKey) {
            ...
            jdbcTemplate.update """
                ...
                ADD CONSTRAINT ${unqualifiedTable}_pk PRIMARY KEY (${primaryKey.join(', ')})
            """, [:]
        }
        ...
        foreignKeys.each { String fkColumn, Map<String, String> refDetails ->
            ...
            jdbcTemplate.update """
                ALTER TABLE ${partitionTable}
                ADD CONSTRAINT ${unqualifiedTable}_${fkColumn}_fk FOREIGN KEY (${fkColumn})
        ...
        indexes.eachWithIndex { List<String> columns, i ->
            def indexName = "${unqualifiedTable}_${i + 1}"
